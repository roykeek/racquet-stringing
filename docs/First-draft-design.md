# Tennis Racquet Stringing Webapp - Design Specification

**Version:** 1.0
**Target Region:** Israel (Hebrew Interface)
...
2. **Action Sheet Opens:**
    ...
    * **Client Actions:**
        * [📞 Call Client] (<code>tel:</code> link).
        * [💬 WhatsApp] (<code>wa.me</code> link).

### Admin: Adding a Stringer

1. Click "Add Stringer".
...
- [ ] Set up layout scaffolding supporting both RTL (he) and LTR (en).
* [ ] Explicitly set `dir="ltr"` on numeric inputs (phone, tension).
* [ ] Seed Database with Initial User (Tomer / 1t2k).
* [ ] Implement Manufacturer -> Model dynamic loading, with "Other" free-text fallback.
* [ ] Ensure Tension inputs (Mains/Crosses) are labeled **Lbs**.
* [ ] Implement client Status Page (UUID routing).
* [ ] Build "Shared Calendar" query (Select all jobs where Date >= Today).
* [ ] Test Mobile "Thumb Zone" layout (Buttons accessible at bottom).
